...
/**
 * 汇总基站数据表 计算每个用户在不同的时间段不同的基站停留的时长 输入参数 < input path > < output path > < date >
 * < timepoint > 参数示例： “/base /output 2012-09-12 09-17-24"
 * 意味着以“/base”为输入，"/output"为输出，指定计算2012年09月12日的数据，并分为00-07，07-17，17-24三个时段 输出格式
 * “IMSI|CGI|TIMFLAG|STAY_TIME”
 */
public class BaseStationDataPreprocess extends Configured implements Tool {
  ...
  public static class Map extends Mapper<LongWritable, Text, Text, Text> {
    ...
    /**
     * MAP任务 读取基站数据 找出数据所对应时间段 以IMSI和时间段作为 KEY CGI和时间作为 VALUE
     */
  ...
  public static void main(String[] args) throws Exception {
    if (args.length != 4) {
      System.err.println("");
      System.err
          .println("Usage: BaseStationDataPreprocess < input path > < output path > < date > < timepoint >");
      System.err
          .println("Example: BaseStationDataPreprocess /user/james/Base /user/james/Output 2012-09-12 07-09-17-24");
      System.err
          .println("Warning: Timepoints should be begined with a 0+ two digit number and the last timepoint should be 24");
      System.err.println("Counter:");
      System.err.println("\t" + "TIMESKIP" + "\t" + "Lines which contain wrong date format");
      System.err.println("\t" + "OUTOFTIMESKIP" + "\t"
          + "Lines which contain times that out of range");
